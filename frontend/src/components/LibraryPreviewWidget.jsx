...
const LibraryPreviewWidget = ({type, settings, w, h, pixelSize}) => {
  const {ref} = useDraggable({id: crypto.randomUUID()})
    const Component = widgetTypes[type]
    const style = pixelSize ? { width: pixelSize.width, height: pixelSize.height} : { width: `${w * 8}cqw`, height: `${h * 8}cqw`}
  return (
...
export default LibraryPreviewWidget
